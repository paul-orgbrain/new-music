...
const hitNames: { [drum: string]: { [hit: string]: string } } = {
  djembe: {
    B: '54018__domingus__djembe-lo-2.wav',
    S: '54015__domingus__djembe-hi-2.wav',
    T: '54022__domingus__djembe-mid-2.wav',
  },
  kenkeni: {
    H: '170179__jimmyjames112__kenkeni.wav',
    M: '170179__jimmyjames112__kenkeni.wav',
  },
  sangban: {
    H: '170177__jimmyjames112__sangban.wav',
    M: '170178__jimmyjames112__sangban-mute.wav',
  },
};

export class DrumPlayer implements Ticker, Instrument {
  name = 'kit';
  samples: { [drum: string]: { [hit: string]: SamplePlayer } } = {};
  type = 'DrumPlayer';
  names: Array<string> = [];
  muted: boolean;
  scoreText: String =
  ...
  constructor(
    ...
    Object.keys(hitNames).forEach((drum) => {
      Object.keys(hitNames[drum]).forEach((hitname) => {
        this.samples[drum] = {};
        samplesService
          .samplePlayer(sampleFolder + hitNames[drum][hitname])
          .then((sp) => {
  ...
  tick() {
    ...
    const ptrNow: number = Math.floor(
      this.pulse.beat * this.pulse.pulsesPerBeat * this.pulse.subDivs[0] + TOL
    );
    ...
    if (this.monitor)
      this.monitor.spareTime(this.pulse.time - audioContext.currentTime);
  }
