...
export class Music extends Savable {
  ...
  constructor(
    ...
    this.band = new Band();
    window.navigator['requestMIDIAccess']().then((midiAccess: any) => {
        ...
        midiInput.onmidimessage = function (event: any) {
          ...
          self.things.forEach((p) => {
            if (p instanceof Player) {
              if (p.recording && p.inst instanceof SFInstrument)
                p.inst.playEvent(event.data, 0);
            }
          });
        };
      });
