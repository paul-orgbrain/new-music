...
export class Metro implements Ticker {
  static _samples: { [key: string]: any } = {};
  ...
  constructor(
    ...
    samplesService
      .samplePlayer(Metro._samples['Clave'])
      .then((sp) => (this.samples[0] = sp));
    samplesService
      .samplePlayer(Metro._samples['Taktell'])
      .then((sp) => (this.samples[1] = sp));
    samplesService
      .samplePlayer(Metro._samples['DanBeat'])
      .then((sp) => (this.samples[2] = sp));
  ...
  tick() {
    ...
    const pulseNow: number = Math.floor(
      this.pulse.beat * this.pulse.pulsesPerBeat + TOL
    );
...
Metro._samples['Taktell'] =
  'assets/sounds/metro/christeck/Metronom Taktell Junior.wav';
